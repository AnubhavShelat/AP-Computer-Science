...
public class BinaryTreeAVL
{
	...
	public TreeNode insert(TreeNode t, Comparable item)
	{
		...
		if (balance(t) <= -2)
		{
			if (balance(t.getRight()) == -1)
				t = rotateRight(t);
			if (balance(t.getRight()) == 1)
			{
				t.setRight(rotateLeft(t.getRight()));
				t = rotateRight(t);
			}
		}
		if (balance(t) >= 2)
		{
			if (balance(t.getLeft()) == -1)
			{
	...
	{
		TreeNode o = t;
		TreeNode p = o.getRight();
		o.setRight(p.getLeft());
		p.setLeft(o);
		return p;
	}
	...
	{
		TreeNode o = t;
		TreeNode p = o.getLeft();
		o.setLeft(p.getRight());
		p.setRight(o);
		return p;
	}
